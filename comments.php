<?php
if(!function_exists('theme_list_comment')){
    function theme_list_comment($comment, $args, $depth) {
        ...
        <div <?php comment_class('comment-item'); ?> id="comment-<?php comment_ID(); ?>">
            ...
            <div class="comment-author">
                ...
                <?php printf('<span class="comment-author-name">%s</span>', $username); ?>
                <?php printf('<span class="comment-date" data-now="%1$s"></span>', get_comment_date()) ?>
            </div>

            <div class="comment-text">

                <?php if ($comment->comment_approved == '0') : ?>
                    <div class="alert alert-info margin-bottom"><?php _e('Your comment is awaiting moderation.', 'elgrecowoo');?></div>
                <?php endif; ?>
            ...
            <div class="comment-meta">
                ...
                <?php edit_comment_link( __( 'Edit', 'elgrecowoo' ), '  ', '') ?>
    ...
    function theme_comment( $comment, $args, $depth ) {
        ...
        <div class="comm_one" id="comment-<?php comment_ID(); ?>">
            ...
            <span class="comm_reply"><?php comment_reply_link( array_merge( $args, [ 'depth' => $depth, 'max_depth' => $args[ 'max_depth' ] ] ) ); ?></span>
            <?php edit_comment_link(__('Edit', 'elgrecowoo'), '  ', '') ?>
        </div>
        <?php
    }
...
//output comments
    ...
    function custom_comment_form( $args = [], $post_id = null ) {
        if ( null === $post_id ) {
            $post_id = get_the_ID();
        }
        ...
        <div class="form-group">
            ...
            <label for="textarea_comm">* '. __('Comment', 'elgrecowoo') .'</label>
        </div>',

            /** This filter is documented in wp-includes/link-template.php */
            'must_log_in'          => '<p class="must-log-in">' . sprintf( __( 'You must be <a href="%s">logged in</a> to post a comment.', 'elgrecowoo' ), wp_login_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',

            /** This filter is documented in wp-includes/link-template.php */
            'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'elgrecowoo' ), get_edit_user_link(), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post_id ) ) ) ) . '</p>',
            ...
            'name_submit'          => 'submit',
            'title_reply'          => __('Leave a comment', 'elgrecowoo'),
            'title_reply_to'       => __('Leave a reply to', 'elgrecowoo') . ' %s',
            'cancel_reply_link'    => __('Cancel reply', 'elgrecowoo'),
            'label_submit'         => __('Post Сomment', 'elgrecowoo'),
            'format'               => 'xhtml',
        ];
                ...
                <?php else : ?>
                    ...
                    <form action="<?php echo site_url( '/wp-comments-post.php' ); ?>" method="post" id="<?php echo esc_attr( $args['id_form'] ); ?>" class="nicelabel comment-form form-comment" data-toggle="validator" role="form">
                        ...
                        <div class="form-group submit_cont">
                            <button type="submit" class="btn btn-secondary"><?php echo esc_attr( $args['label_submit'] ); ?></button>
                        </div>
    ...
    <div class="comm_cont">
        ...
        <?php if (have_comments()) : ?>
            ...
            <h5 class="bigH5"><?php _e( 'Comments', 'elgrecowoo' ) ?> <span>(<?php echo get_comments_number(); ?>)</span></h5>
